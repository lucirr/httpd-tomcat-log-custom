...
if [ "$1" = "debug" ] ; then
  ...
  echo "**************************************"
  echo "org.apache.catalina.startup.Bootstrap $@ start 2>&1 | tee $CATALINA_BASE/logs/${HOSTNAME}/catalina.out"
  echo "**************************************"

  mkdir -p $CATALINA_BASE/logs/${HOSTNAME}/
  touch $CATALINA_BASE/logs/${HOSTNAME}/catalina.out
  ...
  if [ "$1" = "-security" ] ; then
    ...
    eval exec "\"$_RUNJAVA\"" "\"$CATALINA_LOGGING_CONFIG\"" $LOGGING_MANAGER "$JAVA_OPTS" "$CATALINA_OPTS" \
      ...
      -Dcatalina.home="\"$CATALINA_HOME\"" \
      -Djava.io.tmpdir="\"$CATALINA_TMPDIR\"" \
      org.apache.catalina.startup.Bootstrap "$@" start 2>&1 | tee $CATALINA_BASE/logs/${HOSTNAME}/catalina.out
  else
    eval exec "\"$_RUNJAVA\"" "\"$CATALINA_LOGGING_CONFIG\"" $LOGGING_MANAGER "$JAVA_OPTS" "$CATALINA_OPTS" \
      -D$ENDORSED_PROP="\"$JAVA_ENDORSED_DIRS\"" \
      -classpath "\"$CLASSPATH\"" \
